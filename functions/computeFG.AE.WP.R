...
#' 
computeFG.AE.WP <- function(min9.K, eddy.diff.name = "EddyDiff", bootstrap){
  ...
  H2O.FG <- applyeqn.FG.aerowindprof(min9 = H2O, eddy.diff.name = eddy.diff.name,
  ...
  CO2.FG <- applyeqn.FG.aero.windprof(min9 = CO2, eddy.diff.name = eddy.diff.name,
                                     bootstrap=1, nsamp=1000)
  ...
  CH4.FG <- applyeqn.FG.aero.windprof(min9 = CH4, eddy.diff.name = eddy.diff.name,
                                      bootstrap=1, nsamp=1000)
