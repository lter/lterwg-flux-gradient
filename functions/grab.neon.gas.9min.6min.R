...
grab.neon.gas.9min.6min <- function(hd.file, sitecode){
  ...
  #subsets heights list grabbing only those at 9m
  heights <- as.numeric(substr(stringr::str_subset(unique(stringr::str_subset(heights.list, '000')),'09m'),6,6))
  #heights.2m <-unique(stringr::str_subset(heights.co2h2o, '000')) %>% stringr::str_subset( '2m')
  
  #MOVED TO SITEATTRIBUTES FUNCTION
  ...
  for(i in 1:length(heights)){
    ...
    # progress message
    message("Processing height ", i, " (", length(heights) - i, " remaining)")
  ...
  #df_CH4 %>% filter(duplicate_ct > 1)
  # Diagnostic check (DELETE ME)
  #df_CO2 %>% filter(duplicate_ct > 1)
  #df_H2O %>% filter(duplicate_ct > 1)
  ...
  CH4.clean <- df_CH4 %>% 
    ...
    # pivot_wider(names_from = heights, values_from = c(mean, qfFinl))
  ...
  #Merge all data:
  # totF <- H2O.clean %>% left_join(CH4.clean, by= c('timeEnd', 'timeBgn')) %>% left_join(CO2.clean, by= c('timeEnd', 'timeBgn'))
  #totF$timeEnd1 <- time.format(totF$timeEnd)  # Reformat the time and round it by one second
  tower_conc = list(CH4 = CH4.clean, CO2 = CO2.clean, H2O = H2O.clean)
  return(tower_conc)
}


#DEPRECIATED JACKIE CODE
# #h5read fcn reads a dataset in h5 file and returns an array containing data read
