...
# I changed this function to only include the dirs for the sites in the sitecode
...
getYearSiteVisits <- function(downloadDirs,siteCode) {
  ...
  for(i in 1:length(downloadDirs)) {
    ...
    ysv <- dirParts[length(dirParts)-1] 
    if (grepl(siteCode,ysv)) { yearSiteVisits <- append(yearSiteVisits, ysv)
...
getDataDirs <- function(dataRootDir,siteCode,dpID,downloadDirs) {
  ...
  dataDirs <- dataDir[grep(siteCode, dataDir)]  
  return(dataDirs)
}
...
makeFullSiteMosaics <- function(dpID,year,siteCode,dataRootDir,outFileDir,apiToken=NULL) {
  ...
  # This is where data would be downloaded:
  # if (is.null(apiToken)) {
  ...
  if(  length(dataDirs) > 0) { # made a check for dirs > 0
    ...
    for (i in 1:length(dataTiles)) {
      print(i)
      try( fullMosaics[[i]] <- mergeDataTiles(dataTiles[[i]]), silent=T)
      try( dataDirSplit <- unlist(strsplit(dataTiles[[i]][1],.Platform$file.sep)), silent=T)
      try(tileNameSplit <- unlist(strsplit(dataTiles[[i]][1],'_')), silent=T)
      
      if (tail(tileNameSplit,1)=='error.tif') {
        try(fullMosaicNames[[i]] <- paste0(dataDirSplit[9],'_',tail(tileNameSplit,n=2)[1],'_error.tif') , silent=T)} 
      else {
        try( fullMosaicNames[[i]] <- paste0(dataDirSplit[9],'_',tail(tileNameSplit,n=1)) , silent=T)}
    }
    ...
    for (i in 1:length(fullMosaics)) {
      try(outFileTif <- fullMosaicNames[[i]], silent = T)
      try(cat(paste0('Generating ',outFileTif,'\n')), silent = T)
      try( writeFullMosaicTif(fullMosaics[[i]],outFileDir,outFileTif), silent = T)
      # name the COG file the same as tif but with COG suffix
      try( outFileCog <- gsub(".tif", "_COG.tif", outFileTif) , silent = T)
      try(cat(paste0('Generating ',outFileCog,'\n')), silent = T)
      try(convertTif2Cog(outFileDir,outFileTif,outFileCog), silent = T)
    }
