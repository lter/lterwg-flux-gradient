...
files <- c("concentration_2020")

# Read first 5 rows to identify columns
for (file_id in files){
  file_path <- paste0("C:/Users/kyled/Dropbox/Research/NCEAS_flux gradient/Data/SE-Deg/", file_id, ".csv")
  print(file_path)
  dt <- fread(file_path)
  
  # header_sample <- data.table::fread(file_id, nrows = 60*30+10)
  # header_sample <- header_sample[-1]  # first row is units, need to change this
  # 
  ...
  # header_sample[, (columns_to_convert) := lapply(.SD, as.numeric), .SDcols = columns_to_convert]
  # 
  # 
