...
# Define a function that accepts different min/max bounds for different metrics
filterWithBounds <- function(raster, metric_type) {
  bounds <- list(
    "EVI" = list(min = 0, max = 1),
    "NDVI" = list(min = 0, max = 1),
    "LAI" = list(min = 0, max = 12),
    "PRI" = list(min = -0.2, max = 0.2),
    "SAVI" = list(min = -0.6, max = 1.0),
    "CHM" = list(min = 0, max = 60)
  )
  
  min_val <- bounds[[metric_type]]$min
  max_val <- bounds[[metric_type]]$max
  
  raster[raster > max_val] <- NA
  raster[raster < min_val] <- NA
  return(raster)
}
...
for(s in folders){
  ...
  for(raster in evi.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      ...
      try(r2 <- filterWithBounds(r1, "EVI"), silent=T)
...
for(s in folders){
  ...
  for(raster in NDVI.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      try(r1 <- terra::rast(paste(path,raster, sep="/")) %>% terra::project(Sites.wedges.sub), silent=T)
      try(r2 <- filterWithBounds(r1, "NDVI"), silent=T)
...
for(s in folders){
  ...
  for(raster in LAI.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      try(r1 <- terra::rast(paste(path,raster, sep="/")) %>% terra::project(Sites.wedges.sub), silent=T)
      try(r2 <- filterWithBounds(r1, "LAI"), silent=T)  # Updated to use the correct LAI range
...
for(s in folders){
  ...
  for(raster in PRI.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      try(r1 <- terra::rast(paste(path,raster, sep="/")) %>% terra::project(Sites.wedges.sub), silent=T)
      try(r2 <- filterWithBounds(r1, "PRI"), silent=T)  # Updated to use the correct PRI range
...
for(s in folders){
  ...
  for(raster in CHM.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      try(r1 <- terra::rast(paste(path,raster, sep="/")) %>% terra::project(Sites.wedges.sub), silent=T)
      try(r2 <- filterWithBounds(r1, "CHM"), silent=T)  # Updated to use the correct CHM range
...
for(s in folders){
  ...
  for(raster in SAVI.files){
    ...
    if(length(Sites.wedges.sub$Name) > 0){
      try(r1 <- terra::rast(paste(path,raster, sep="/")) %>% terra::project(Sites.wedges.sub), silent=T)
      try(r2 <- filterWithBounds(r1, "SAVI"), silent=T)  # Updated to use the correct SAVI range
