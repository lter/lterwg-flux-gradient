...
for( site in unique(SITES_One2One$Site) ){
  ...
  SITES_MBR_30min_FILTER_BH[[site]] <-  SITES_MBR_30min_FILTER[[site]] %>% full_join( BH.MBR, by= c( 'site','gas') ) %>% filter(dLevelsAminusB == BestHeight) 
  
  
  BH.AE <- Best_Level %>% filter(Site == site, Approach=='AE') %>% mutate(site = Site)  
  SITES_AE_30min_FILTER_BH[[site]] <-  SITES_AE_30min_FILTER[[site]]  %>% full_join( BH.AE, by= c( 'site','gas') ) %>% filter(dLevelsAminusB == BestHeight) 
  
  BH.WP <- Best_Level %>% filter(Site == site, Approach=='WP') %>% mutate(site = Site)  
  
  SITES_WP_30min_FILTER_BH[[site]] <-  SITES_WP_30min_FILTER[[site]]  %>% full_join( BH.WP, by= c( 'site','gas') ) %>% filter(dLevelsAminusB == BestHeight) 
}
