...
plot.CCC.CO2.f100 <- SITES_One2One_canopy %>% filter(gas=="CO2", count >100) %>%  ggplot() + geom_point( aes(x= CCC, y=Site, col=Approach ), alpha= 0.7, size = 3) + facet_grid(cols = vars(Canopy_L1)) + scale_color_manual(values=c("goldenrod", "aquamarine4", "darkmagenta")) + theme_bw() + ylab("") 
...
plot.CCC.CO2.t0.5.f100 <- SITES_One2One_canopy %>% filter(gas=="CO2", CCC > 0.50, count >100) %>%  ggplot() + geom_point( aes(x= CCC, y=Site, col=Approach , shape= Canopy_L1), alpha= 0.7, size = 3) + scale_color_manual(values=c("goldenrod", "aquamarine4", "darkmagenta")) + theme_bw() + ylab("")
...
plot.CCC.H2O.f100 <- SITES_One2One_canopy %>% filter(gas=="H2O", count >100) %>%  ggplot() + geom_point( aes(x= CCC, y=Site, col=Approach ), alpha= 0.7, size = 3) + facet_grid(cols = vars(Canopy_L1)) + scale_color_manual(values=c("goldenrod", "aquamarine4", "darkmagenta")) + theme_bw() + ylab("") 
...
plot.CCC.H2O.t0.5.f100 <- SITES_One2One_canopy %>% filter(gas=="H2O", CCC> 0.50, count >100) %>%  ggplot() + geom_point( aes(x= CCC, y=Site, col=Approach , shape= Canopy_L1), alpha= 0.7, size = 3) + scale_color_manual(values=c("goldenrod", "aquamarine4", "darkmagenta")) + theme_bw() + ylab("")

# Count Evaluation:

Summary.CCC.t0.5.f100 <- SITES_One2One_canopy %>% filter( CCC> 0.50, count > 100) %>% reframe( .by=c(Canopy_L2, gas),
...
counts.CO2.CCC.f100 <- SITES_One2One_canopy %>% filter( CCC> 0.50, count > 100, gas=="CO2") %>% ggplot(aes(x= Approach, y=Good.CCC)) + geom_bar(stat = "identity") + theme_bw() + ylab ( 'Measurement Levels (CCC > 0.5)')    
...
counts.H2O.CCC.f100 <- SITES_One2One_canopy %>% filter( CCC> 0.50, count > 100, gas=="H2O") %>% ggplot(aes(x= Approach, y=Good.CCC)) + geom_bar(stat = "identity") + theme_bw() + ylab ( 'Measurement Levels (CCC > 0.5)')    
...
localdir <- '/Volumes/MaloneLab/Research/FluxGradient/FluxData'
setwd(localdir)
save(plot.CCC.CO2.all,  plot.CCC.CO2.f100 , plot.CCC.CO2.t0.5 ,
     plot.CCC.H2O.all,  plot.CCC.H2O.f100 , plot.CCC.H2O.t0.5 ,
     plot.countbyCCC, plot.countbyCCC,plot.countbyApproach,
     counts.CO2.CCC.f100 ,counts.CO2.CCC, counts.CO2.CCC.approach,
     counts.H2O.CCC.f100 ,counts.H2O.CCC, counts.H2O.CCC.approach,
     SITES_One2One_canopy, SITES_One2One_canopy.Highest, SITES_One2One_canopy.Highest.100,
     Summary.CCC.t0.5.f100, Summary.CCC.t0.5, file = "flow.CCC_VIZ.R" )
