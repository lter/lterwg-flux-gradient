...
Flux_Gradient_AE <- function(cont.df, attr.df, z1_height, z2_height, z_height){
  ...
  #FG AE calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_ch4_AE)] <- (1.293*as.numeric(ae.K))*(Conc_CH4_z1-Conc_CH4_z2/z1-z2)
  ...
  #FG AE calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_co2_AE)] <- (1.293*as.numeric(ae.K))*(Conc_co2_z1-Conc_co2_z2/z1-z2)
  ...
  #FG AE calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_LE_AE)] <- (1.293*as.numeric(ae.K))*(Conc_h2o_z1-Conc_h2o_z2/z1-z2)
  ...
  #FG WP calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_ch4_AE_WP)] <- (1.293*as.numeric(wp.K))*(Conc_CH4_z1-Conc_CH4_z2/z1-z2)
  
  #calculate co2 flux using WP
  #FG WP calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_co2_AE_WP)] <- (1.293*as.numeric(wp.K))*(Conc_co2_z1-Conc_co2_z2/z1-z2)
  
  #calculate LE flux using WP
  #FG WP calculation: need the unit conversion constant for molar to mass mixing ratio, assuming rho (air density) is 1.293 kg m−3
  ae.df[,which(names(ae.df) == F_LE_AE_WP)] <- (1.293*as.numeric(wp.K))*(Conc_h2o_z1-Conc_h2o_z2/z1-z2)
