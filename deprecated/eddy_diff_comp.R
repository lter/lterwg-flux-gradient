...
outliers_MBR <- function(data,x){
  quant <- quantile(data$FCO2_MBR_H2Otrace,prob=c(.25,.75),na.rm = TRUE)
  IQR <- quant[2]-quant[1]
  IQR <- IQR*x
  low <- quant[1]- IQR
  high <- quant[2] + IQR
...
df_CO2_MBR<- df_CO2_MBR  %>% filter(df_CO2_MBR$dLevelsAminusB_CO2 == "5_4")
...
plot(df_CO2_MBR_DIURNAL$hour,df_CO2_MBR_DIURNAL$mean_MBR_flux)