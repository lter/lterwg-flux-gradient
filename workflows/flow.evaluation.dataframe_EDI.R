...
save.data <-'/Volumes/MaloneLab/Research/FluxGradient/NEON_GradientFlux_EDI'
...
for( site in site.list){
  ...
  # Load the files:
  localdir.site <- paste(data.local.dir,"/NEON_GradientFlux_Data/", site, sep = "")
  ...
  dir.create(paste(save.data,"/",site, sep="" ))
  
  save.data.site <- paste(save.data,"/",site, sep="" )

    # Save the files

  write.csv(MBR_9min.df.final, paste0(save.data.site,"/", site,"_MBR_9min.df.final.csv" ) )
  write.csv(AE_9min.df.final, paste0(save.data.site,"/", site,"_AE_9min.df.final.csv" ) )
  write.csv(WP_9min.df.final, paste0(save.data.site,"/", site,"_WP_9min.df.final.csv" ) )
